...
test_that("ewaa data can be added to model", {
  ...
  ewaa_growth$ages$fromR(get_ages(fims_frame))
  ewaa_growth$weights$fromR(m_weight_at_age(fims_frame))
  expect_equal(ewaa_growth$evaluate(1), 0.00053065552)

  ewaa_growth2 <- methods::new(EWAAgrowth)
  ewaa_growth2$ages$fromR(c(get_ages(fims_frame), 12))
  ewaa_growth2$weights$fromR(m_weight_at_age(fims_frame))
  expect_error(
