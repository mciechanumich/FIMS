...
public:
    ...
    SharedPrimitive() : value(std::make_shared<T>(0.0)) {
    }

    SharedPrimitive(T val) : value(std::make_shared<T>(val)) {
    }

    // Copy Constructor

    SharedPrimitive(const SharedPrimitive& other) : value(other.value) {
    }

    // Assignment Operator

    SharedPrimitive& operator=(const SharedPrimitive& other) {
        if (this != &other) {
            value = other.value;
        }
        return *this;
    ...
    SharedPrimitive& operator=(const T& other) {
    ...
    SharedPrimitive(SharedPrimitive&& other) noexcept : value(std::move(other.value)) {
    }

    // Move Assignment Operator

    SharedPrimitive& operator=(SharedPrimitive&& other) noexcept {
        if (this != &other) {
    ...
    SharedPrimitive& operator++() {
        ++(*value);
        return *this;
    } // Prefix increment

    SharedPrimitive operator++(int) {
        SharedPrimitive temp(*this);
        ++(*value);
        return temp;
    } // Postfix increment
    ...
    SharedPrimitive operator+(const SharedPrimitive& other) const {
        return SharedPrimitive(*value + *other.value);
    }
    ...
    SharedPrimitive operator+(const T& other) const {
        return SharedPrimitive(*value + other);
    }
    ...
    bool operator==(const SharedPrimitive& other) const {
    ...
    friend std::ostream& operator<<(std::ostream& os, const SharedPrimitive& sp) {
        os << *sp.value;
        return os;
    }
};
...
template <typename T>
SharedPrimitive<T> operator+(const T& lhs, const SharedPrimitive<T>& rhs) {
    return SharedPrimitive<T>(lhs + rhs.get());
}
...
RCPP_EXPOSED_CLASS(SharedPrimitive<int>)
RCPP_EXPOSED_CLASS(SharedPrimitive<double>)


typedef SharedPrimitive<uint32_t> fims_uint32_t;
typedef SharedPrimitive<int> fims_int;
typedef SharedPrimitive<double> fims_double;
