...
default_parameters <- data_4_model |>
  create_default_parameters(
    fleets = create_default_fleet_list(data_4_model)
  )
```
...
`purrr::map_vec()` can be used to compare the length of individual updated parameter vectors from `paramters` with the length of individual parameter vectors from `default_parameters`.
